...
# ---------------------------------------------------------------------------
# 検証済みファクト（2026-07-28 時点）
#   一次情報: BitMart公式「Important Notice Regarding the Orderly Cessation of BitMart Operations」
# 本文の数値・固有名詞はこの範囲のみを根拠とする（推測の数値を足さない）。
# ---------------------------------------------------------------------------
HEADLINE = "BitMart、取引プラットフォームを段階的に終了へ　8月26日に全取引停止"

FACTS = """
- 暗号資産取引所BitMartは2026年7月26日、取引プラットフォームの運営を秩序立って終了する方針を公式に発表した。決定理由として、事業運営の状況、市場環境、今後の戦略を総合的に検討したとしている。
- 2026年7月26日1時30分（UTC、日本時間10時30分）以降、新規ユーザー登録と暗号資産・法定通貨の入金受付を段階的に停止している。入金停止後は、口座へ自動反映されない可能性があるため、入金しないよう注意を呼びかけている。
- 同時期から、先物口座は決済のみ可能なReduce-Onlyモードとなり新規建てができなくなる。現物取引では新規注文の受付を停止し、コピートレード、グリッド取引、API取引などの自動取引サービスも段階的に終了する。未約定注文は利用者自身で取り消すか、システムにより取り消される。
- 2026年8月26日1時（UTC、日本時間10時）に、現物・先物を含むすべての取引サービスを停止する予定。未決済の先物ポジションは、当時適用されるマーク価格、指数価格、または決済ルールに基づき、プラットフォーム側で決済される可能性がある。詳細な決済方法は別途案内される。
- BitMart Earn、ステーキング、レンディング、Launchpadなどのサービスも、それぞれの取り扱いに応じて段階的に終了する。償還・決済などの手続きは、対象者に個別の告知およびプラットフォーム内通知で案内される。
- 出金サービスは発表後も利用可能としている。BitMartは、8月26日1時（UTC）までに本人確認と保有ポジションの解消を済ませ、同日5時（UTC、日本時間14時）より前に出金申請を行うよう強く推奨している。
- プラットフォーム運営は2027年1月31日15時59分（UTC、日本時間2月1日0時59分）に正式終了する予定。終了後も一定期間はログインして残高や履歴を確認し、定められた手続きに従って出金申請できるとしている。
"""

SOURCES_NOTE = "（出典: BitMart公式発表「Important Notice Regarding the Orderly Cessation of BitMart Operations」を基に編集部が作成）"
