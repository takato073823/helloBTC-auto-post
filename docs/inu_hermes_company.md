...
発見シグナルに入れる。GitHub Actions上の定時運用ではxAI公式APIのX Searchを使用し、
HermesはProfiles・Kanban・部門監査を担当する。どちらのX投稿も最終出典にせず、後段が
公式発表・IR・規制当局・公式データへ戻る。

## 品質と費用の両立

- 無料の公式API、RSS、オンチェーン、市場データは毎枠確認する。
- xAI X SearchとOpenAI Web Searchは4時間ごとの深い探索に限定する。
- xAI文章編集は、一次情報で検証済みの採用候補だけに使う。
- 日次上限はxAI探索6回、xAI編集12回、OpenAI Web Search 8回、AI画像6枚。
- 各呼び出しは実行前に費用枠を確保し、失敗した呼び出しも上限へ算入する。
- `inu_research_review.json`に探索経路、採用シグナル数、日次使用数を保存し、改善を比較可能にする。
